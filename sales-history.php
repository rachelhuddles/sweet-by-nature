...
<!--
	...
	sales-history.html
	For accessing a record of previous sales 
-->
<html lang="en">
<head>
	<title>Sales History | Sweet by Nature, Inc.</title> 
	<meta charset="utf-8">
...
<nav class="navbar navbar-expand-lg navbar-dark">
	...
	<div class="main-links border-top border-light">
		...
		<?php
			...
			$sql = "SELECT * FROM Orders ORDER BY OrderID DESC LIMIT 20";
			$result = $conn->query($sql);

			if ($result->num_rows > 0) {
				// output data of each row
				while($row = $result->fetch_assoc()) {
				echo "Order ID: " . $row["OrderID"]. "<br>Company ID: " . 
				$row["CompanyID"]. "<br>Order Date: " . $row["OrderDate"]
				. "<br>Product:  " . $row["Product"]. "<br>Amount Ordered: " . $row["OrderAmount"] . "<br><br>";
				}
			} else {
				echo "0 results";
			}
