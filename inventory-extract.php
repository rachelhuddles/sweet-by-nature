<?PHP if (!session_id()) session_start(); if (!$_SESSION[ 'logon']){ header( "Location:index.html"); die(); } require_once( 'database.php'); $queryExtractInventory='Select ExtractID, AddDate, Amount
    From extractInventory 
    Order By ExtractID' ; $statement=$ db -> prepare($queryExtractInventory); $statement -> execute(); $eInv = $statement -> fetchAll(); $statement -> closeCursor(); ?>
...
<!--
	...
	inventory-extract.php
	Honeybush Extract Inventory
-->
<html lang="en">
...
<body>
    ...
    <div class="main-links border-top border-light">
        <h1 class="text-center">Honeybush Extract Inventory</h1>
        ...
        <table cellpadding="0" cellspacing="0" style="width: 100%">
            ...
            <?php foreach ($eInv as $e) { ?>
            <tr>
                <td>
                    <?php echo $e[ 'ExtractID']; ?>
                </td>
                <td>
                    <?php echo $e[ 'AddDate']; ?>
                </td>
                <td>
                    <?php echo $e[ 'Amount']; ?> lbs.</td>
            </tr>
            <?php } ?>
    </div>
    <!-- Section: Links grid-->
